                ...
                <div class="p-6 sm:p-8 bg-white dark:bg-gray-800">
                    <form method="POST" action="{{ route('stock-movements.store') }}" class="space-y-6">
                                ...
                                <select id="product_id" name="product_id" required class="block w-full mt-1 rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:focus:ring-indigo-800 transition duration-150 ease-in-out">
                                    ...
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
                                            {{ $product->name }} (SKU: {{ $product->sku }} | Qty: {{ $product->quantity }})
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="quantity" class="block w-full mt-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-800 shadow-sm" type="number" name="quantity" :value="old('quantity')" min="1" required />
